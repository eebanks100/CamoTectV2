...
# Elisha - load ground truth if there is ground truth else set it to none lines 182 - 224 (was originally to perform validation using test dataset without having to use ground truth images)
    ...
    def load_data(self):
        ...
        original_size = cv2.imread(self.images[self.index]).shape[:2]

        # if self.gts is None:
        #     return image, name, original_size, np.array(image_for_post)
        if self.gts is None:
            return image, name, original_size, np.array(image_for_post)
        return image, gt, name, original_size, np.array(image_for_post)
